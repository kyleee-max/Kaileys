# ClientPayload

## Namespaces

- [DNSSource](namespaces/DNSSource/index.md)
- [UserAgent](namespaces/UserAgent/index.md)
- [WebInfo](namespaces/WebInfo/index.md)

## Enumerations

- [AccountType](enumerations/AccountType.md)
- [ConnectReason](enumerations/ConnectReason.md)
- [ConnectType](enumerations/ConnectType.md)
- [IOSAppExtension](enumerations/IOSAppExtension.md)
- [Product](enumerations/Product.md)
- [TrafficAnonymization](enumerations/TrafficAnonymization.md)

## Classes

- [DevicePairingRegistrationData](classes/DevicePairingRegistrationData.md)
- [DNSSource](classes/DNSSource.md)
- [InteropData](classes/InteropData.md)
- [UserAgent](classes/UserAgent.md)
- [WebInfo](classes/WebInfo.md)

## Interfaces

- [IDevicePairingRegistrationData](interfaces/IDevicePairingRegistrationData.md)
- [IDNSSource](interfaces/IDNSSource.md)
- [IInteropData](interfaces/IInteropData.md)
- [IUserAgent](interfaces/IUserAgent.md)
- [IWebInfo](interfaces/IWebInfo.md)
