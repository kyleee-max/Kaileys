# InteractiveMessage

## Namespaces

- [CarouselMessage](namespaces/CarouselMessage/index.md)
- [NativeFlowMessage](namespaces/NativeFlowMessage/index.md)
- [ShopMessage](namespaces/ShopMessage/index.md)

## Classes

- [Body](classes/Body.md)
- [CarouselMessage](classes/CarouselMessage.md)
- [CollectionMessage](classes/CollectionMessage.md)
- [Footer](classes/Footer.md)
- [Header](classes/Header.md)
- [NativeFlowMessage](classes/NativeFlowMessage.md)
- [ShopMessage](classes/ShopMessage.md)

## Interfaces

- [IBody](interfaces/IBody.md)
- [ICarouselMessage](interfaces/ICarouselMessage.md)
- [ICollectionMessage](interfaces/ICollectionMessage.md)
- [IFooter](interfaces/IFooter.md)
- [IHeader](interfaces/IHeader.md)
- [INativeFlowMessage](interfaces/INativeFlowMessage.md)
- [IShopMessage](interfaces/IShopMessage.md)
